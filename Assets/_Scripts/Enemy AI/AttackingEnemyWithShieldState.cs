...
public class AttackingEnemyWithShieldState : EnemyState {
	...
	private float initialShieldDelayTimer = 10f;
	private float shieldDelayTimer;
	private float initialShieldTimer = 2f;
	private float shieldTimer;

	public AttackingEnemyWithShieldState(Enemy enemy, Player player)
	{
		this.enemy = enemy;
	...
	private FancyEnemy fancyEnemy;

	public override void StartState() 
	{
		fancyEnemy = enemy.GetComponent<FancyEnemy>();
		shieldTimer = initialShieldTimer;
		moveTimer = initialMoveTimer;
		shieldDelayTimer = initialShieldDelayTimer;
	...
	public override void Execute() 
	{
		...
		if(fancyEnemy.Shield.activeInHierarchy)
		{
			shieldTimer -= Time.deltaTime;
		}
		else
		{
			initialShieldDelayTimer -= Time.deltaTime;
		}

		if(!fancyEnemy.Shield.activeInHierarchy && initialShieldDelayTimer < 0)
		{
			fancyEnemy.Shield.SetActive(true);
			shieldTimer = initialShieldTimer;
		}

		if(fancyEnemy.Shield.activeInHierarchy && initialShieldTimer < 0)
		{
			fancyEnemy.Shield.SetActive(false);
			shieldDelayTimer = initialShieldDelayTimer;
		}
